...
static void get_motor_bemf(void);
...
static void apply_smooth_start(void){
	...
	*/
	...
	if((ui8_throttle_adc_in * SMOOTH_START_THROTTLE_FACTOR) > smooth_start_voltage_limit_x100){
		smooth_start_voltage_limit_x100 = (uint16_t)ui8_throttle_adc_in * SMOOTH_START_THROTTLE_FACTOR;
	}

	uint16_t smooth_start_voltage_limit_blend_x100 = 0;
	uint16_t temp_blend = 0;
	#if 1
	//ramp away the limit for strong pedal press
...
static void apply_rev_matching(void){
	...
	if ((ui8_riding_torque_mode == 1U) && (ui8_pedal_cadence_RPM > 0U)){
		...
		if (pedal_sync_duty_cycle_target > PWM_DUTY_CYCLE_MAX){
			pedal_sync_duty_cycle_target = PWM_DUTY_CYCLE_MAX;
		}
		uint8_t cadence_motor_pedal_delta;
...
static void apply_speed_limit(void)
{
    if (m_configuration_variables.ui8_wheel_speed_max > 0U) {
        ...
        ui8_adc_battery_current_target = map_ui16((uint16_t) ui16_wheel_speed_x10,
                ((uint16_t)m_configuration_variables.ui8_wheel_speed_max * 10U) - 20U,
                ((uint16_t)m_configuration_variables.ui8_wheel_speed_max * 10U) + 20U,
                ui8_adc_battery_current_target,
                0);
		if (ui16_wheel_speed_x10 > (((uint16_t)m_configuration_variables.ui8_wheel_speed_max * 10U) + 20U)) {
			ui8_duty_cycle_target = 0;
		}
    }
}
...
static void calc_wheel_speed(void)
{
	...
	// !!!warning if MOTOR_TASK_FREQ may not be a multiple of 1000
	ui16_wheel_speed_x10 = (m_configuration_variables.ui16_wheel_perimeter << 2U) * 36U / ui16_wheel_speed_sensor_ticks * ((MOTOR_TASK_FREQ >> 2U) / 1000U);
}
...
static void get_motor_bemf(void){
	if(ui8_fw_hall_counter_offset==0U) { //bemf should not increase when field-weakening is enabled
		//the worst case scenario is for 48V motor where max speed before overflow is 
		//780erps = 5850rpm which equates to 140rpm cadence (from 41.8 gearing)
...
#define TOFFSET_CYCLES 120 // 3sec (25ms*120)
    ...
    if (ui16_adc_pedal_torque > ui16_adc_pedal_torque_offset) {
		...
		// ui16_adc_pedal_torque_delta = (ui16_adc_pedal_torque_delta + ui16_adc_pedal_torque_delta_temp) >> 1; //average with previous value
	}
	else {
