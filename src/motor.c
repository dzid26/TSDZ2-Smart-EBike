...
static uint8_t ui8_adc_motor_phase_current = 0;
...
void HALL_SENSOR_A_PORT_IRQHandler(void)  __interrupt(EXTI_HALL_A_IRQ) {
    ...
    if (((HALL_SENSOR_A__PORT->IDR) & GPIO_PIN_5) > 0U){
        ui8_hall_state_irq |= 0x01U;
        if (((HALL_SENSOR_B__PORT->IDR & HALL_SENSOR_B__PIN) > 0U)){ //forward
            i8_motor_erotations_since_cadence_tick++;
        }
        if ((HALL_SENSOR_C__PORT->IDR & HALL_SENSOR_C__PIN) > 0U){ //reverse
            i8_motor_erotations_since_cadence_tick--;
        }
    }else{
        ui8_hall_state_irq &= ~0x01U;
